MakeLateArrHist <- function(FlightDelays05 = FlightDelays05, carrier= "all"){
  if(carrier == "all"){
    ...
    quant <-quantile(FlightDelays05$LATE_ARR[FlightDelays05[,carrier]==1], probs = c(.01, .99))
    plot<- ggplot(data = FlightDelays05[which(FlightDelays05$LATE_ARR<quant[2] & FlightDelays05[,carrier] == 1) ,], aes(x = LATE_ARR)) + 
      geom_histogram(bins = 25) + 
