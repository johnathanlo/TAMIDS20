...
%%%
		\subsection{Making Up Lost Time En Route}
			\tab One last thing we wanted to explore was the ability for pilots to make up lost time as a result of a departure delay. We presumed that a longer flight, distance-wise, would allow for more opportunity for a flight delay to be alleviated. In the air, harnessing favorable air currents or taking shortcuts can remedy the lost time they left the ground with. Surely, in \underline{Fig 9.X}, we see that flights over a longer distance have, on average, less arrival delays. Similarly, the shorter the flight, the more severe the arrival delay is. These observations support our hypothesis stated above. Of course, correlation does not imply causation, so more substantive knowledge on how pilots navigate the flight route would provide more clarification on this. \\
...
%%
	...
	\tab Using this bootstrap time series dataset, we fitted a multi-seasonal ARIMA time series model. The decomposition of this time series into constituent seasonalities and trends can be seen in \underline{Fig 9.X}. Then, taking the residuals from our time series and appending it back to the original data, we fitted a linear model to predict these residuals using our covariates that did not involve time (since we already accounted for this in our time-based model). Our goal in fitting another linear model to the residuals was to try and explain the variation in the time-series that could not be picked up by the seasonalities. The resulting model produced residuals that were approximately random (\underline{Fig 9.X}), or at least more random than our previous models. Our \textit{$R^2$} was 0.369; 36.9\% of the variation in our data could be explained by the model we fit. After analyzing the plots of our covariates, we saw that there was a slight non-linear relationship with precipitation (the variance was decreasing as the precipitation was increasing) (\underline{Fig 9.X}), so we decided to correct for this by weighting observations by the square root of precipitation. After this, our \textit{$R^2$} increased to 0.48 (\underline{Fig 9.X}).\\
...
%%
	...
	\tab However, despite our limitations, we were able to construct a dynamic linear model that explained just over 50\% of the variance in arrival delays. Our model was fit by averaging the coefficients generated by OLS on 5 separate samples of 50,000 observations. 
%%
...
%/////////////////////////////////CHAPTER 4 [EDA] FIGURES
			...
			\begin{figure}[h]
			\centering
	 		\includegraphics[width = 0.5\textwidth]{../figures/cde_latitude}
	 		\caption{conditional densities of arrival delays across different latitudes}
	 		\end{figure}

			\begin{figure}[h]
			\centering
	 		\includegraphics[width = 0.5\textwidth]{../figures/cde_longitude}
			...
			\begin{figure}[h]
			\centering
	 		\includegraphics[width = 1 \textwidth]{../figures/CDE_prcp}
...
%/////////////////////////////////CHAPTER 5 [MODEL FORMULATION AND ASSESSMENT] FIGURES
	 		...
	 		\begin{figure}[h]
			\centering
	 		\includegraphics[width =.7 \textwidth]{../figures/shittysqrt}
	 		...
	 		\begin{figure}[h]
			\centering
	 		\includegraphics[width = .6\textwidth]{../figures/MarginalMLEHist}
	 		\caption{Top: Our estimated Lo-Ke distribution fit of marginal density. Top: Distribution of the actual marginal data. We see that our theoretical Lo-Ke distribution with estimated parameters very well captures the empirical distribution. }
	 		\end{figure}

			\begin{figure}[h]
			\centering
	 		\includegraphics[width = .8 \textwidth]{../figures/QQplot-marginaldata}
	 		\caption{QQ plot of of data vs theoretical Lo-Ke distribution. As the points follow the lines y=x, we see that our distribution almost exactly matches the distribution of the data.}
	 		\end{figure}		
			...
			\begin{figure}[h]
			\centering
	 		\includegraphics[width = .85\textwidth]{../figures/PLOTS FOR REPORT/Logistic Regression/summary(logit_reg)}
			...
			\begin{figure}[h]
			\centering
	 		\includegraphics[width = .65\textwidth]{../figures/PLOTS FOR REPORT/Logistic Regression/confusion matrix - logit reg}
			...
			\centering
	 		...
	 		\caption{Multiseasonal time series decomposition for arrival time using bootstrap method}
	 		\end{figure}

			\begin{figure}[h]
			\centering
	 		\includegraphics[width = 1 \textwidth]{../figures/plot1_residualLM}
	 		\caption{Plot of residuals vs. fitted values for our linear model to account for the residuals in our time-based model}
	 		\end{figure}

			\begin{figure}[h]
			\centering
	 		\includegraphics[width = 1 \textwidth]{../figures/non_weighted_lm_output}
	 		\caption{R output for our linear model on the residuals of our time series}
	 		\end{figure}

			\begin{figure}[h]
			\centering
	 		\includegraphics[width = 1 \textwidth]{../figures/weighted_lm_output}
	 		\caption{R output for our weighted linear model on the residuals of our time series}
	 		\end{figure}
